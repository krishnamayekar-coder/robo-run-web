...
export function Header({ 
  ...
  const [internalTimeFilter, setInternalTimeFilter] = useState<TimeFilterOption>(() => {
    const saved = localStorage.getItem('dashboard-time-filter');
    return (saved as TimeFilterOption) || 'current-sprint';
  });

  const [internalSelectedProject, setInternalSelectedProject] = useState<string>(() => {
  ...
  useEffect(() => {
    if (externalSelectedProject) {
      localStorage.setItem('dashboard-selected-project', externalSelectedProject);
    }
  }, [externalSelectedProject]);
  
  const userEmail = gitData?.user_record?.email || "user@company.com";
  const userRole = gitData?.user_role || gitData?.user_record?.role || localStorage.getItem('userRole') || "USER";
      ...
      <div className="flex h-16 items-center justify-between px-3 sm:px-4 md:px-6 w-full">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 glass-card rounded-xl px-3 py-2 h-10">
            <img src={logo} alt="AI WorkTracker" className="h-7 w-7 sm:h-8 sm:w-8 shrink-0" />
            <div className="flex flex-col min-w-0">
              <span className="font-heading text-sm sm:text-base font-bold text-foreground truncate">AI WorkTracker</span>
              <span className="text-[10px] text-muted-foreground uppercase tracking-wider hidden sm:block">Team Analytics</span>
            </div>
          </div>
        </div>
            ...
            <Select value={timeFilter} onValueChange={handleTimeFilterChange}>
              <SelectTrigger className="w-[140px] sm:w-[160px] border-0 bg-transparent shadow-none focus:ring-0 h-auto py-0">
                <div className="flex items-center gap-2">
                  ...
                  <SelectValue>
                    {TIME_FILTER_OPTIONS.find(opt => opt.value === timeFilter)?.label || "Current Sprint"}
                  </SelectValue>
                </div>
              </SelectTrigger>
              <SelectContent className="glass-dropdown">
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="relative rounded-full p-0 h-8 w-8">
                  <Avatar className="h-8 w-8 border-2 border-primary/20">
                    <AvatarFallback className="bg-primary text-primary-foreground font-medium text-xs">{userInitials}</AvatarFallback>
                  </Avatar>
                </Button>
              </DropdownMenuTrigger>
